...
const AlaCarteMain = ({image, image2, header, text, text2, span, subImages=[], lang="en"}) => {
  return (
      <section className="flex w-screen h-[70vh] items-start justify-between bg-white">
        <div className="flex flex-col w-full lg:w-[49%] items-center justify-center lg:gap-[20px] text-center lg:text-start ">
         ...
         <img
          ...
          <h3 className="absolute bottom-2 z-10 left-1/2 -translate-x-1/2 text-shadow-custom flex text-[25px] text-white font-lora font-medium leading-normal uppercase lg:text-[40px]">{header[lang]}</h3>
         </div>
          
          <div className="flex w-[80%] lg:w-[70%] items-center justify-center gap-[28px] mt-[43px] text-center">
            <img src={subImages[0].firebaseUrl} width={33} height={39} color="#CFCFCF"/>
            <img src={subImages[1].firebaseUrl} width={37} height={40} color="#CFCFCF"/>
            <img src={subImages[2].firebaseUrl} width={29} height={39} color="#CFCFCF"/>
          </div>
        ...
        <img
          ...
          <p className="text-[15px] w-[90%] lg:w-[55%] text-black font-monserrat font-normal leading-[22.5px]"> {text2[lang]}</p>
        </div>
