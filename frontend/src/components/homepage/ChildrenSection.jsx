  ...
  return (
    <div className='flex flex-col w-screen items-center justify-center gap-[50px] mt-10'>
      <div className='flex flex-col w-[90%] lg:w-[60%] gap-[30px] items-center justify-center text-center'>
        <h2 className='text-[#233038] text-[25px] lg:text-[40px] font-lora font-medium leading-normal'>{header[lang]}</h2>
      ...
      {items.map((item, index) => (
        <div key={index} className={item.wrapperClasses}>
          ...
          <span className='text-[14px] text-[#233038CC] leading-normal font-bold'>{item.text[lang]}</span>
        </div>
      ))}
    </div>
      </div>
      <ChildrenCarousel images={images}/>
      <button className='flex bg-white border border-[#868686] py-[12px] px-[31px] font-monserrat font-bold text-[14px] text-center text-[#233038] leading-normal hover:bg-[#233038] hover:text-[#fff]'>{buttonText[lang]}</button>
    </div>
  )
}
