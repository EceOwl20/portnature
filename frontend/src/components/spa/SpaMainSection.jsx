...
const SpaMainSection = ({image, image2, header, text, span, buttonText, buttonLink, iconImage}) => {
  return (
    ...
    <div className='flex flex-col justify-center items-center text-center py-[40px]'>
        ...
        <Link to={buttonLink} className='flex bg-white border py-[10px] px-[34px] justify-center items-center text-[16px] leading-normal font-monserrat font-bold text-customGray gap-[14.67px]'><img src={iconImage.firebaseUrl} alt='youtube' />{buttonText}</Link>
      </div>
    </div>
      <div className='flex w-full items-center justify-center gap-[104px]'>
        <div className='flex justify-center items-center py-[35px] px-[107.5px] border border-[#CFCFCF] gap-[15px] text-[28px] font-lora italic font-normal text-customGray80 leading-[42px]'>
