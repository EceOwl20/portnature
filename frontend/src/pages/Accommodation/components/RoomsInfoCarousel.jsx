        ...
        <div className='flex flex-col mt-0 lg:mt-10 w-screen mb-[20px] lg:mb-[66px] relative'>
          <div className='flex flex-col gap-[20px] items-center justify-center w-full text-[#233038]'>
            <div className=' flex-col hidden gap-[23px] text-center text-darkB w-full items-center justify-center'>
              <span className='text-[20px] tracking-[5px] leading-normal font-lora uppercase font-medium mb-[25px]'>{header}</span>
              <p className='text-[14px] font-semibold leading-[22.5px] text-start text-stoneLight'>{text}</p>
              <div className="grid grid-cols-3  items-center justify-center w-full">
                ...
                <img src={items[0].firebaseUrl} width={43} height={48} color="#3D515E"/>
                   ...
                   <span className="text-[20px] font-lora font-medium leading-[42px] text-[#3D515E]">{items[0].text}</span>
                </div>
                <div className="flex items-center justify-center gap-2">
                <img src={items[1].firebaseUrl} width={29} height={48} color="#3D515E"/>
                    <Cross2Svg width={20} height={20}/>
                   <span className="text-[20px] font-lora font-medium leading-[42px] text-[#3D515E]"> {items[1].text}</span>
                </div>
                <div className="flex items-center justify-center gap-2">
                   <span className="text-[20px] font-lora font-medium leading-[42px] text-[#3D515E]"> {items[2].text} </span>
                </div>
          ...
          <div className='flex flex-col gap-3 lg:gap-[23px] text-center md:text-start text-darkB w-full lg:w-11/12 items-center justify-center'>
              <span className='text-[20px] lg:text-[40px] leading-normal uppercase font-lora font-medium lg:mb-[0px]'>{header}</span>
              {/* <NewUnderline width={170} height={1}/> */}
              <Underline />
              <p className='text-[12px] lg:text-[15px] font-normal leading-[22.5px] text-start font-monserrat text-black'>{text}</p>
              <div className="grid grid-cols-3  items-center justify-center w-full">
                <div className="flex items-center justify-center gap-2">
            ...
            <div className="flex grid-flow-col">
              {images.map((image, index) => (
                <div className="relative flex mx-1 flex-[0_0_auto] border-[2px] border-gray-100 md:max-h-[571px] shadow-lg xl:w-[calc(70%-1.4rem)] xl:mx-3 " key={index}>
                  <img src={image.firebaseUrl} alt="image" width={image.width} height={image.height} className="object-cover" />
