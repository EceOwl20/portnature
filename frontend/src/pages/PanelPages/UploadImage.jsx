...
const UploadImage = () => { 
    ...
    useEffect(() => {
        if (firebaseUrl) {
          handleUploadfirebaseUrl(firebaseUrl);
        }
      }, [firebaseUrl]);
    
      const handleUploadfirebaseUrl = (image) => {
        setWait(true);
      ...
      const handleSubmit = async (e) => {
        ...
        // `formData`'nın eksiksiz olduğundan emin olun
        if (!formData.firebaseUrl || Object.values(formData.name).some((v) => !v) || Object.values(formData.altText).some((v) => !v)) {
        ...
        try {
          const response = await fetch("http://localhost:3000/api/images/upload", {
            ...
            body: JSON.stringify(formData), // Form verilerini gönderin
          });
    ...
    <div className="flex flex-col items-center justify-center font-monserrat">
      ...
      <input
       className="flex border border-[#0E0C1B] py-2 p-2 w-[57%] bg-white rounded-md"
        ...
        name="firebaseUrl"
        accept="image/*"
        onChange={(e) => setFirebaseUrl(e.target.files[0])}
        required
        disabled={wait}
      />
